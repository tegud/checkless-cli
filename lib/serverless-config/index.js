...
const snsTopics = {
    makeRequestTopic: "${self:custom.makeRequestTopic}",
...
module.exports = {
    expandToServerlessConfig: (config) => {
        ...
        const custom = {
            makeRequestTopic: `${service}-make-request`,
        ...
        const additionalFunctions = config.httpEnabled ? {
            "trigger-check": triggerCheck(snsTopics),
        } : undefined;

        coreServerlessConfig[homeRegion] = {
            service,
            custom,
            provider: provider({ region: homeRegion, service, httpEnabled: config.httpEnabled }, "nodejs8.10"),
            functions: {
